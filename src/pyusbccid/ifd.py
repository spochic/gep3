...
class InterfaceDevice:
    def __init__(self, device: usb.core.Device, interface: usb.core.Interface, cfg_index: int, interface_index: int, slot: int = 0):
        ...
        if self.device.is_kernel_driver_active(self.interface_index):
            logging.debug("Detaching kernel driver")
            self._reattach = True
            self.device.detach_kernel_driver(self.interface_index)
    ...
    def send_bulk_message(self, message: BulkOutMessage, timeout=None) -> BulkInMessage:
        ...
        if n != len(message.array):
            raise AssertionError(
                F"{n} bytes written, expecting {len(message.arra)}")
